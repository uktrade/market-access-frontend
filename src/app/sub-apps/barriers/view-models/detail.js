...
function buildLocationString( country, adminAreas){ 
	adminAreasString = adminAreas ? adminAreas.map(
		(adminAreaId) => metadata.getAdminArea(adminAreaId)['name']
	).join(', ') : "";
	return adminAreasString ? `${adminAreasString} (${country.name})` : country.name;
}
...
module.exports = ( barrier, addCompany = false ) => {
	...
	return {
		...
		barrier: {
			...
			location: buildLocationString(metadata.getCountry( barrier.export_country ), barrier.country_admin_areas),
			sectors,
			source: {
				id: barrier.source,
