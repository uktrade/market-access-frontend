ma.components.DatahubHeader = (function( doc, helpers ){

	if( !( helpers.toggleClass && helpers.hasListenerSupport ) ){ return; }
	...
	DatahubHeader.prototype.handleClick = function(){
		...
		var isAriaHidden = ( this.toggleElems[ 0 ].getAttribute( 'aria-hidden' ) === 'false' );

		helpers.toggleClass( this.$module, CONTAINER_OPEN_CLASS );
		helpers.toggleClass( this.$toggleButton, TOGGLE_BUTTON_OPEN_CLASS );

		this.$toggleButton.setAttribute( 'aria-expanded', this.$toggleButton.getAttribute( 'aria-expanded' ) !== 'true' );
	...
	var html = isHostObjectProperty( doc, 'documentElement' ) && doc.documentElement;
	...
	if( helpers.hasClass && helpers.addClass && helpers.removeClass ){
		helpers.toggleClass = function ( el, className ){
			var toggle = helpers.hasClass( el, className ) ? 'removeClass' : 'addClass';
